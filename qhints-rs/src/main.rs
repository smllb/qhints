...
fn cull_and_relabel(
    ...
    // Sort by label length (keep shorter first)
    let mut indices: Vec<usize> = (0..items.len()).collect();
    indices.sort_by_key(|&i| items[i].0.len());
    
    // Cull overlapping hints
    let overlap_threshold = 0.2;
    ...
    for &i in &indices {
        ...
        for &j in &indices {
            ...
            if ix1 < ix2 && iy1 < iy2 {
                ...
                if min_area > 0.0 && intersection / min_area > overlap_threshold {
                    if items[i].0.len() <= items[j].0.len() {
                        keep[j] = false;
                    } else {
                        keep[i] = false;
                        break;
    ...
    // Relabel with zone-based 2-char labels only (no 1-char to avoid conflicts)
